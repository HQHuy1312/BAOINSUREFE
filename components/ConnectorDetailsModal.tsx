...
const ConnectorDetailsModal: React.FC<ConnectorDetailsModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center transition-opacity duration-300" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-lg m-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto mb-6">
          {connections.length > 0 ? (
            connections.map((conn) => (
              <div key={conn.id} className="p-4 border border-brand-gray-200 rounded-lg bg-brand-gray-100 flex justify-between items-center">
                <div>
